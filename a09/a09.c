/* A09, 6809 Assembler.
   ...
   Revisions:
        ...
        (fork)  Intel Hex output support added (outmode 2, -x option,
                flushihex/outihex, ":00000001FF" end record).
        2026-07-09
                Brought up to ISO/IEC 9899:2018 (C17):
                - added standard #includes (stdio/stdlib/string/ctype);
                - added full forward prototypes (fixes implicit declarations
                  for the recursive scanexpr and mutually recursive
                  processfile/pseudoop);
                - empty parameter lists () changed to (void);
                - processfile changed from int (never returned a value,
                  undefined behavior under C99+) to void;
                - main: getchar() result stored in int, added return 0;
                - ctype.h calls given (unsigned char) casts to avoid
                  undefined behavior on negative char arguments;
                - RESOLVECAT macro rewritten as a brace block (the original
                  relied on a comment spliced into the macro body);
                - hexbuffer enlarged 16 -> 32 bytes: outihex() fills up to
                  32 bytes before flushing, which overflowed the original
                  16-byte buffer;
                - (suppress==1|suppress==2) changed to logical || in
                  suppressline (identical truth value, silences warning).
                Behavior otherwise preserved bug-for-bug, including the
                missing break fall-through after case '|' in scanexpr
                (also present in the 1993/94 original).
*/
